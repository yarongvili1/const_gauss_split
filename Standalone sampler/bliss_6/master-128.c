/*
the fragmented sampler. i.e the columns are divided and the coresponding minimization was calculated in chunks of 4 at a time
*/

//Compile : gcc -O3 -o sample master-128.c -lcrypto

//compile with shake :  gcc -Wall -Wextra -O3 -fomit-frame-pointer -march=native -o sample master-128.c
...
//--------------------12----------------------
		SHA512_Update(&sha512, current_seed1+12, 1);
		SHA512_Final(hash+64*12, &sha512);
//--------------------13----------------------
		SHA512_Update(&sha512, current_seed1+13, 1);
		SHA512_Final(hash+64*13, &sha512);
//--------------------14----------------------
		SHA512_Update(&sha512, current_seed1+14, 1); 
		SHA512_Final(hash+64*14, &sha512);
//--------------------15----------------------
		SHA512_Update(&sha512, current_seed1+15, 1);
		SHA512_Final(hash+64*15, &sha512);
*/
//-------------------sha ends--------------------------
		clock1 = cpucycles();
